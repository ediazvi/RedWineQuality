...
!!!!!!!!!!!!!  HE CAMBIADO EL LISTADO DE VARIABLES SELECCIONADAS, PODEMOS PONER EL QUE QUERAMOS, PERO HEMOS DE TENER ALGUNA LÓGICA CON LOS RESULTADOS ANTERIORES  !!!!
!!!!!!!!!!!!!  DUDA, EL APARTADO NO DEBERÍA SER "REGRESIÓN LINEAL" ???
...
modelo_reducido <- (lm(formula = quality ~ alcohol + sulphates + volatile.acidity, data = datos_clean))
...
````{r }
# Generamos el modelo
#modelo_log <- glm(category ~ fixed.acidity + citric.acid + alcohol + sulphates + volatile.acidity, data = datos_clean, family = "binomial")
...
Lo correcto sería crear un conjunto de entrenamiento y otro de test para el modelo.
```{r }
set.seed(1234)
trainIndex <- createDataPartition(datos_clean$quality, p = 0.8, list = FALSE)
train_data <- datos_clean[trainIndex, ]
test_data <- datos_clean[-trainIndex, ]

#modelo_log_1 <- glm(quality_factor ~ alcohol + sulphates + volatile.acidity, data = train_data, family = "binomial")
modelo_log_1 <- glm(quality ~ volatile.acidity + I(log10(chlorides)) + 
    free.sulfur.dioxide + total.sulfur.dioxide + pH + I(log10(sulphates)) + 
    alcohol, data = train_data, family = "gaussian")
...
```{r }
#predicciones_1 <- ifelse(test = modelo_log_1$fitted.values > 0.5, yes = 1, no = 0)
predicciones_1 <- predict(modelo_log_1, test_data)
plot(test_data$quality, predicciones_1)

matriz_confusion_1 <- table(test_data$quality , round(predicciones_1), dnn = c("observaciones", "predicciones"))
matriz_confusion_1



```
En este caso observamos como el modelo tiene un 60% de acierto en la predicción.
